...
function Sidebar({ params }) {
  ...
  return (
    <div className='z-20'>
    <div className="hidden md:flex flex-col items-center justify-normal gap-40 bg-secondary w-72 h-full pt-6 rounded-3xl">
      <img className="w-48 h-16" src="/icon.png" alt="Icon"></img>
      <div className="flex flex-col justify-between h-72">
        {menuItems.map((menuItem, index) => (
          <Link
            key={index}
    ...
    <div onClick={toggleBar} className="md:hidden z-20">
				{!isMenuOpen ? (
					<GiHamburgerMenu className="h-8 w-8 md:hidden" />
				) : (
					<IoClose className="h-10 w-10 md:hidden" />
				)}
			</div>
      {/* Mobile Menu */}
      <ul
				className={
          isMenuOpen
            ? 'absolute z-10 pt-1 bg-secondary flex flex-col justify-center items-center gap-10 text-xl flex-wrap w-screen h-screen md:hidden'
            : 'hidden'
        }>
				{menuItems.map((menuItem, index) => (
          <Link
